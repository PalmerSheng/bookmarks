...
async function translateWithCloudflareAI(text, targetLang = 'Chinese') {
  if (!text || text.trim() === '') {
    return '';
  }
  ...
  try {
    const url = `https://api.cloudflare.com/client/v4/accounts/${CLOUDFLARE_ACCOUNT_ID}/ai/run/@cf/meta/llama-4-scout-17b-16e-instruct`;
...
serve(async (req)=>{
  ...
  try {
    ...
    if (method === 'ai' || method === 'cloudflare') {
      ...
      const aiTargetLang = targetLang === 'zh-CN' ? 'Chinese' : targetLang === 'en' ? 'English' : targetLang === 'ja' ? 'Japanese' : targetLang === 'ko' ? 'Korean' : targetLang === 'fr' ? 'French' : targetLang === 'de' ? 'German' : targetLang === 'es' ? 'Spanish' : 'Chinese';
